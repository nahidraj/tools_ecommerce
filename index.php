<?php
include("website-layout/header.php");
?>

<!-- banner part start -->
<section class="banner">
  <div class="banner_slider" style="background: url(images/banner.jpg);">
    <div class="overlay">
      <div class="container"></div>
    </div>
  </div>
  <div class="banner_slider" style="background: url(images/banner.jpg);">
    <div class="overlay">
      <div class="container"></div>
    </div>
  </div>
  <div class="banner_slider" style="background: url(images/banner.jpg);">
    <div class="overlay">
      <div class="container"></div>
    </div>
  </div>
</section>
<!-- banner part end -->
<!-- category part start -->
<section class="common_widget_box">
  <div class="container">
    <!-- common title -->
    <div class="row">
      <div class="co-lg-12">
        <h4 class="common_title">All Departments</h4>
      </div>
    </div>
    <!-- all category -->
    <div class="common_widget_box_main_with_bg">
      <div class="row g-1">
        <div class="col-lg-2 col-sm-4">
          <div class="common_category_widget text-center">
            <a href="" class="d-block w-100">
              <div class="icon">
                <img src="images/1.png" alt="1" width="60">
              </div>
              <div class="title">
                <h4>Calibration</h4>
              </div>
            </a>
          </div>
        </div>
        <div class="col-lg-2 col-sm-4">
          <div class="common_category_widget text-center">
            <a href="" class="d-block w-100">
              <div class="icon">
                <img src="images/1.png" alt="1" width="60">
              </div>
              <div class="title">
                <h4>Education, Research
                  & Development</h4>
              </div>
            </a>
          </div>
        </div>
        <div class="col-lg-2 col-sm-4">
          <div class="common_category_widget text-center">
            <a href="" class="d-block w-100">
              <div class="icon">
                <img src="images/1.png" alt="1" width="60">
              </div>
              <div class="title">
                <h4>Fault Testing &
                  Diagnostics</h4>
              </div>
            </a>
          </div>
        </div>
        <div class="col-lg-2 col-sm-4">
          <div class="common_category_widget text-center">
            <a href="" class="d-block w-100">
              <div class="icon">
                <img src="images/1.png" alt="1" width="60">
              </div>
              <div class="title">
                <h4>Insulation,
                  Resistance and Battery</h4>
              </div>
            </a>
          </div>
        </div>
        <div class="col-lg-2 col-sm-4">
          <div class="common_category_widget text-center">
            <a href="" class="d-block w-100">
              <div class="icon">
                <img src="images/1.png" alt="1" width="60">
              </div>
              <div class="title">
                <h4>Mechanical &
                  Maintenance</h4>
              </div>
            </a>
          </div>
        </div>
        <div class="col-lg-2 col-sm-4">
          <div class="common_category_widget text-center">
            <a href="" class="d-block w-100">
              <div class="icon">
                <img src="images/1.png" alt="1" width="60">
              </div>
              <div class="title">
                <h4>Pharma,
                  Health & Biomedical</h4>
              </div>
            </a>
          </div>
        </div>
        <div class="col-lg-2 col-sm-4">
          <div class="common_category_widget text-center">
            <a href="" class="d-block w-100">
              <div class="icon">
                <img src="images/1.png" alt="1" width="60">
              </div>
              <div class="title">
                <h4>Calibration</h4>
              </div>
            </a>
          </div>
        </div>
        <div class="col-lg-2 col-sm-4">
          <div class="common_category_widget text-center">
            <a href="" class="d-block w-100">
              <div class="icon">
                <img src="images/1.png" alt="1" width="60">
              </div>
              <div class="title">
                <h4>Education, Research
                  & Development</h4>
              </div>
            </a>
          </div>
        </div>
        <div class="col-lg-2 col-sm-4">
          <div class="common_category_widget text-center">
            <a href="" class="d-block w-100">
              <div class="icon">
                <img src="images/1.png" alt="1" width="60">
              </div>
              <div class="title">
                <h4>Fault Testing &
                  Diagnostics</h4>
              </div>
            </a>
          </div>
        </div>
        <div class="col-lg-2 col-sm-4">
          <div class="common_category_widget text-center">
            <a href="" class="d-block w-100">
              <div class="icon">
                <img src="images/1.png" alt="1" width="60">
              </div>
              <div class="title">
                <h4>Insulation,
                  Resistance and Battery</h4>
              </div>
            </a>
          </div>
        </div>
        <div class="col-lg-2 col-sm-4">
          <div class="common_category_widget text-center">
            <a href="" class="d-block w-100">
              <div class="icon">
                <img src="images/1.png" alt="1" width="60">
              </div>
              <div class="title">
                <h4>Mechanical &
                  Maintenance</h4>
              </div>
            </a>
          </div>
        </div>
        <div class="col-lg-2 col-sm-4">
          <div class="common_category_widget text-center">
            <a href="" class="d-block w-100">
              <div class="icon">
                <img src="images/1.png" alt="1" width="60">
              </div>
              <div class="title">
                <h4>Pharma,
                  Health & Biomedical</h4>
              </div>
            </a>
          </div>
        </div>
      </div>
    </div>
  </div>
</section>
<!-- category part end -->

<!-- brand part start -->
<section class="common_widget_box">
  <div class="container">
    <!-- common title -->
    <div class="row">
      <div class="co-lg-12">
        <h4 class="common_title">Brands</h4>
      </div>
    </div>
    <!-- all brands -->
    <div class="common_widget_box_main_without_bg">
      <div class="row brands_slider g-1">
        <div class="col-lg-2">
          <div class="common_brand_widget">
            <a href="" class="d-block w-100">
              <div class="img">
                <img src="https://tools.com.bd/public/storage/brand/fluke-logopng.PNG" alt="brand-1" class="img-fluid w-100">
              </div>
            </a>
          </div>
        </div>
        <div class="col-lg-2">
          <div class="common_brand_widget">
            <a href="" class="d-block w-100">
              <div class="img">
                <img src="https://tools.com.bd/public/storage/brand/meggerpng.png" alt="brand-2" class="img-fluid w-100">
              </div>
            </a>
          </div>
        </div>
        <div class="col-lg-2">
          <div class="common_brand_widget">
            <a href="" class="d-block w-100">
              <div class="img">
                <img src="https://tools.com.bd/public/storage/brand/amprobepng.png" alt="brand-1" class="img-fluid w-100">
              </div>
            </a>
          </div>
        </div>
        <div class="col-lg-2">
          <div class="common_brand_widget">
            <a href="" class="d-block w-100">
              <div class="img">
                <img src="https://tools.com.bd/public/storage/brand/opal-rtpng.png" alt="brand-2" class="img-fluid w-100">
              </div>
            </a>
          </div>
        </div>
        <div class="col-lg-2">
          <div class="common_brand_widget">
            <a href="" class="d-block w-100">
              <div class="img">
                <img src="https://tools.com.bd/public/storage/brand/testo-brandpng.png" alt="brand-1" class="img-fluid w-100">
              </div>
            </a>
          </div>
        </div>
        <div class="col-lg-2">
          <div class="common_brand_widget">
            <a href="" class="d-block w-100">
              <div class="img">
                <img src="https://tools.com.bd/public/storage/brand/indelec-in-bdpng.png" alt="brand-2" class="img-fluid w-100">
              </div>
            </a>
          </div>
        </div>
        <div class="col-lg-2">
          <div class="common_brand_widget">
            <a href="#" class="d-block w-100">
              <div class="img">
                <img src="https://tools.com.bd/public/storage/brand/amprobepng.png" alt="brand-1" class="img-fluid w-100">
              </div>
            </a>
          </div>
        </div>
      </div>
    </div>
  </div>
</section>
<!-- brand part end -->

<!-- trendy items part start -->
<section class="common_widget_box">
  <div class="container">
    <!-- common title -->
    <div class="row">
      <div class="co-lg-12">
        <h4 class="common_title">Tredy Items</h4>
      </div>
    </div>
    <!-- all brands -->
    <div class="common_widget_box_main_without_bg">
      <div class="row g-1">
        <div class="col-lg-3 col-xl-2 col-sm-6 col-md-4">
          <div class="common_product_widget">
            <span class="discount">10% Off</span>
            <div class="image">
              <a href="" class="d-block w-100">
                <img src="images/2.jpg" alt="2" class="img-fluid w-100">
              </a>
            </div>
            <div class="text text-center">
              <a href="" class="product_title line_limit_2">The title should be within
                two line</a>
              <div class="rating">
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
              </div>
              <span>BDT: <small>00.00</small></span>

              <div class="product_action d-flex justify-content-between align-items-center">
                <button class="wishlist"><i class="fa-regular fa-heart"></i></button>
                <a href="" class="get_quote">Get Quote</a>
                <button class="cart"><i class="fa-solid fa-cart-shopping"></i></button>
              </div>
            </div>
          </div>
        </div>
        <div class="col-lg-3 col-xl-2 col-sm-6 col-md-4">
          <div class="common_product_widget">
            <span class="discount">10% Off</span>
            <div class="image">
              <a href="" class="d-block w-100">
                <img src="images/3.jpg" alt="3" class="img-fluid w-100">
              </a>
            </div>
            <div class="text text-center">
              <a href="" class="product_title line_limit_2">The title should be within
                two line</a>
              <div class="rating">
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
              </div>
              <span>BDT: <small>00.00</small></span>

              <div class="product_action d-flex justify-content-between align-items-center">
                <button class="wishlist"><i class="fa-regular fa-heart"></i></button>
                <a href="" class="get_quote">Get Quote</a>
                <button class="cart"><i class="fa-solid fa-cart-shopping"></i></button>
              </div>
            </div>
          </div>
        </div>
        <div class="col-lg-3 col-xl-2 col-sm-6 col-md-4">
          <div class="common_product_widget">
            <span class="discount">10% Off</span>
            <div class="image">
              <a href="" class="d-block w-100">
                <img src="images/2.jpg" alt="2" class="img-fluid w-100">
              </a>
            </div>
            <div class="text text-center">
              <a href="" class="product_title line_limit_2">The title should be within
                two line</a>
              <div class="rating">
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
              </div>
              <span>BDT: <small>00.00</small></span>

              <div class="product_action d-flex justify-content-between align-items-center">
                <button class="wishlist"><i class="fa-regular fa-heart"></i></button>
                <a href="" class="get_quote">Get Quote</a>
                <button class="cart"><i class="fa-solid fa-cart-shopping"></i></button>
              </div>
            </div>
          </div>
        </div>
        <div class="col-lg-3 col-xl-2 col-sm-6 col-md-4">
          <div class="common_product_widget">
            <span class="discount">10% Off</span>
            <div class="image">
              <a href="" class="d-block w-100">
                <img src="images/3.jpg" alt="3" class="img-fluid w-100">
              </a>
            </div>
            <div class="text text-center">
              <a href="" class="product_title line_limit_2">The title should be within
                two line</a>
              <div class="rating">
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
              </div>
              <span>BDT: <small>00.00</small></span>

              <div class="product_action d-flex justify-content-between align-items-center">
                <button class="wishlist"><i class="fa-regular fa-heart"></i></button>
                <a href="" class="get_quote">Get Quote</a>
                <button class="cart"><i class="fa-solid fa-cart-shopping"></i></button>
              </div>
            </div>
          </div>
        </div>
        <div class="col-lg-3 col-xl-2 col-sm-6 col-md-4">
          <div class="common_product_widget">
            <span class="discount">10% Off</span>
            <div class="image">
              <a href="" class="d-block w-100">
                <img src="images/2.jpg" alt="2" class="img-fluid w-100">
              </a>
            </div>
            <div class="text text-center">
              <a href="" class="product_title line_limit_2">The title should be within
                two line</a>
              <div class="rating">
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
              </div>
              <span>BDT: <small>00.00</small></span>

              <div class="product_action d-flex justify-content-between align-items-center">
                <button class="wishlist"><i class="fa-regular fa-heart"></i></button>
                <a href="" class="get_quote">Get Quote</a>
                <button class="cart"><i class="fa-solid fa-cart-shopping"></i></button>
              </div>
            </div>
          </div>
        </div>
        <div class="col-lg-3 col-xl-2 col-sm-6 col-md-4">
          <div class="common_product_widget">
            <span class="discount">10% Off</span>
            <div class="image">
              <a href="" class="d-block w-100">
                <img src="images/3.jpg" alt="3" class="img-fluid w-100">
              </a>
            </div>
            <div class="text text-center">
              <a href="" class="product_title line_limit_2">The title should be within
                two line</a>
              <div class="rating">
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
              </div>
              <span>BDT: <small>00.00</small></span>

              <div class="product_action d-flex justify-content-between align-items-center">
                <button class="wishlist"><i class="fa-regular fa-heart"></i></button>
                <a href="" class="get_quote">Get Quote</a>
                <button class="cart"><i class="fa-solid fa-cart-shopping"></i></button>
              </div>
            </div>
          </div>
        </div>
        <div class="col-lg-3 col-xl-2 col-sm-6 col-md-4">
          <div class="common_product_widget">
            <span class="discount">10% Off</span>
            <div class="image">
              <a href="" class="d-block w-100">
                <img src="images/2.jpg" alt="2" class="img-fluid w-100">
              </a>
            </div>
            <div class="text text-center">
              <a href="" class="product_title line_limit_2">The title should be within
                two line</a>
              <div class="rating">
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
              </div>
              <span>BDT: <small>00.00</small></span>

              <div class="product_action d-flex justify-content-between align-items-center">
                <button class="wishlist"><i class="fa-regular fa-heart"></i></button>
                <a href="" class="get_quote">Get Quote</a>
                <button class="cart"><i class="fa-solid fa-cart-shopping"></i></button>
              </div>
            </div>
          </div>
        </div>
        <div class="col-lg-3 col-xl-2 col-sm-6 col-md-4">
          <div class="common_product_widget">
            <span class="discount">10% Off</span>
            <div class="image">
              <a href="" class="d-block w-100">
                <img src="images/3.jpg" alt="3" class="img-fluid w-100">
              </a>
            </div>
            <div class="text text-center">
              <a href="" class="product_title line_limit_2">The title should be within
                two line</a>
              <div class="rating">
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
              </div>
              <span>BDT: <small>00.00</small></span>

              <div class="product_action d-flex justify-content-between align-items-center">
                <button class="wishlist"><i class="fa-regular fa-heart"></i></button>
                <a href="" class="get_quote">Get Quote</a>
                <button class="cart"><i class="fa-solid fa-cart-shopping"></i></button>
              </div>
            </div>
          </div>
        </div>
        <div class="col-lg-3 col-xl-2 col-sm-6 col-md-4">
          <div class="common_product_widget">
            <span class="discount">10% Off</span>
            <div class="image">
              <a href="" class="d-block w-100">
                <img src="images/2.jpg" alt="2" class="img-fluid w-100">
              </a>
            </div>
            <div class="text text-center">
              <a href="" class="product_title line_limit_2">The title should be within
                two line</a>
              <div class="rating">
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
              </div>
              <span>BDT: <small>00.00</small></span>

              <div class="product_action d-flex justify-content-between align-items-center">
                <button class="wishlist"><i class="fa-regular fa-heart"></i></button>
                <a href="" class="get_quote">Get Quote</a>
                <button class="cart"><i class="fa-solid fa-cart-shopping"></i></button>
              </div>
            </div>
          </div>
        </div>
        <div class="col-lg-3 col-xl-2 col-sm-6 col-md-4">
          <div class="common_product_widget">
            <span class="discount">10% Off</span>
            <div class="image">
              <a href="" class="d-block w-100">
                <img src="images/3.jpg" alt="3" class="img-fluid w-100">
              </a>
            </div>
            <div class="text text-center">
              <a href="" class="product_title line_limit_2">The title should be within
                two line</a>
              <div class="rating">
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
              </div>
              <span>BDT: <small>00.00</small></span>

              <div class="product_action d-flex justify-content-between align-items-center">
                <button class="wishlist"><i class="fa-regular fa-heart"></i></button>
                <a href="" class="get_quote">Get Quote</a>
                <button class="cart"><i class="fa-solid fa-cart-shopping"></i></button>
              </div>
            </div>
          </div>
        </div>
        <div class="col-lg-3 col-xl-2 col-sm-6 col-md-4">
          <div class="common_product_widget">
            <span class="discount">10% Off</span>
            <div class="image">
              <a href="" class="d-block w-100">
                <img src="images/2.jpg" alt="2" class="img-fluid w-100">
              </a>
            </div>
            <div class="text text-center">
              <a href="" class="product_title line_limit_2">The title should be within
                two line</a>
              <div class="rating">
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
              </div>
              <span>BDT: <small>00.00</small></span>

              <div class="product_action d-flex justify-content-between align-items-center">
                <button class="wishlist"><i class="fa-regular fa-heart"></i></button>
                <a href="" class="get_quote">Get Quote</a>
                <button class="cart"><i class="fa-solid fa-cart-shopping"></i></button>
              </div>
            </div>
          </div>
        </div>
        <div class="col-lg-3 col-xl-2 col-sm-6 col-md-4">
          <div class="common_product_widget">
            <span class="discount">10% Off</span>
            <div class="image">
              <a href="" class="d-block w-100">
                <img src="images/3.jpg" alt="3" class="img-fluid w-100">
              </a>
            </div>
            <div class="text text-center">
              <a href="" class="product_title line_limit_2">The title should be within
                two line</a>
              <div class="rating">
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
              </div>
              <span>BDT: <small>00.00</small></span>

              <div class="product_action d-flex justify-content-between align-items-center">
                <button class="wishlist"><i class="fa-regular fa-heart"></i></button>
                <a href="" class="get_quote">Get Quote</a>
                <button class="cart"><i class="fa-solid fa-cart-shopping"></i></button>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</section>
<!-- trendy items part end -->

<!-- all products part start -->
<section class="common_widget_box">
  <div class="container">
    <!-- common title -->
    <div class="row">
      <div class="co-lg-12">
        <h4 class="common_title">All Products</h4>
      </div>
    </div>
    <!-- all brands -->
    <div class="common_widget_box_main_without_bg">
      <div class="row g-1">
        <div class="col-lg-3 col-xl-2 col-sm-6 col-md-4">
          <div class="common_product_widget">
            <span class="discount">10% Off</span>
            <div class="image">
              <a href="" class="d-block w-100">
                <img src="images/2.jpg" alt="2" class="img-fluid w-100">
              </a>
            </div>
            <div class="text text-center">
              <a href="" class="product_title line_limit_2">The title should be within
                two line</a>
              <div class="rating">
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
              </div>
              <span>BDT: <small>00.00</small></span>

              <div class="product_action d-flex justify-content-between align-items-center">
                <button class="wishlist"><i class="fa-regular fa-heart"></i></button>
                <a href="" class="get_quote">Get Quote</a>
                <button class="cart"><i class="fa-solid fa-cart-shopping"></i></button>
              </div>
            </div>
          </div>
        </div>
        <div class="col-lg-3 col-xl-2 col-sm-6 col-md-4">
          <div class="common_product_widget">
            <span class="discount">10% Off</span>
            <div class="image">
              <a href="" class="d-block w-100">
                <img src="images/3.jpg" alt="3" class="img-fluid w-100">
              </a>
            </div>
            <div class="text text-center">
              <a href="" class="product_title line_limit_2">The title should be within
                two line</a>
              <div class="rating">
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
              </div>
              <span>BDT: <small>00.00</small></span>

              <div class="product_action d-flex justify-content-between align-items-center">
                <button class="wishlist"><i class="fa-regular fa-heart"></i></button>
                <a href="" class="get_quote">Get Quote</a>
                <button class="cart"><i class="fa-solid fa-cart-shopping"></i></button>
              </div>
            </div>
          </div>
        </div>
        <div class="col-lg-3 col-xl-2 col-sm-6 col-md-4">
          <div class="common_product_widget">
            <span class="discount">10% Off</span>
            <div class="image">
              <a href="" class="d-block w-100">
                <img src="images/2.jpg" alt="2" class="img-fluid w-100">
              </a>
            </div>
            <div class="text text-center">
              <a href="" class="product_title line_limit_2">The title should be within
                two line</a>
              <div class="rating">
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
              </div>
              <span>BDT: <small>00.00</small></span>

              <div class="product_action d-flex justify-content-between align-items-center">
                <button class="wishlist"><i class="fa-regular fa-heart"></i></button>
                <a href="" class="get_quote">Get Quote</a>
                <button class="cart"><i class="fa-solid fa-cart-shopping"></i></button>
              </div>
            </div>
          </div>
        </div>
        <div class="col-lg-3 col-xl-2 col-sm-6 col-md-4">
          <div class="common_product_widget">
            <span class="discount">10% Off</span>
            <div class="image">
              <a href="" class="d-block w-100">
                <img src="images/3.jpg" alt="3" class="img-fluid w-100">
              </a>
            </div>
            <div class="text text-center">
              <a href="" class="product_title line_limit_2">The title should be within
                two line</a>
              <div class="rating">
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
              </div>
              <span>BDT: <small>00.00</small></span>

              <div class="product_action d-flex justify-content-between align-items-center">
                <button class="wishlist"><i class="fa-regular fa-heart"></i></button>
                <a href="" class="get_quote">Get Quote</a>
                <button class="cart"><i class="fa-solid fa-cart-shopping"></i></button>
              </div>
            </div>
          </div>
        </div>
        <div class="col-lg-3 col-xl-2 col-sm-6 col-md-4">
          <div class="common_product_widget">
            <span class="discount">10% Off</span>
            <div class="image">
              <a href="" class="d-block w-100">
                <img src="images/2.jpg" alt="2" class="img-fluid w-100">
              </a>
            </div>
            <div class="text text-center">
              <a href="" class="product_title line_limit_2">The title should be within
                two line</a>
              <div class="rating">
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
              </div>
              <span>BDT: <small>00.00</small></span>

              <div class="product_action d-flex justify-content-between align-items-center">
                <button class="wishlist"><i class="fa-regular fa-heart"></i></button>
                <a href="" class="get_quote">Get Quote</a>
                <button class="cart"><i class="fa-solid fa-cart-shopping"></i></button>
              </div>
            </div>
          </div>
        </div>
        <div class="col-lg-3 col-xl-2 col-sm-6 col-md-4">
          <div class="common_product_widget">
            <span class="discount">10% Off</span>
            <div class="image">
              <a href="" class="d-block w-100">
                <img src="images/2.jpg" alt="2" class="img-fluid w-100">
              </a>
            </div>
            <div class="text text-center">
              <a href="" class="product_title line_limit_2">The title should be within
                two line</a>
              <div class="rating">
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
                <i class="fa-solid fa-star"></i>
              </div>
              <span>BDT: <small>00.00</small></span>

              <div class="product_action d-flex justify-content-between align-items-center">
                <button class="wishlist"><i class="fa-regular fa-heart"></i></button>
                <a href="" class="get_quote">Get Quote</a>
                <button class="cart"><i class="fa-solid fa-cart-shopping"></i></button>
              </div>
            </div>
          </div>
        </div>

      </div>
    </div>
  </div>
</section>
<!-- all products part end -->

<?php
include("website-layout/footer.php");
?>